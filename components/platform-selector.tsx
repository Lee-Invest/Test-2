          ...
          <div
            ...
            className={`relative flex flex-col gap-2 rounded-2xl border-2 p-4 transition ${
              !allowed
                ...
                : selected
                ? "cursor-pointer border-[var(--brand-primary)] bg-[var(--brand-primary)]/10"
                : "cursor-pointer border-gray-200 bg-white hover:border-gray-300"
            }`}
            onClick={() => allowed && onSelect(selected ? null : platform.id, feeCents)}
          >
            {selected && allowed && (
              <div className="absolute right-3 top-3 flex h-5 w-5 items-center justify-center rounded-full bg-[var(--brand-primary)] text-white">
                <Check size={12} />
              </div>
            )}
